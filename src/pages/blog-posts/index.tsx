...
type ColumnType = ColumnDef<BlogPostInterface, unknown>;
...
export function BlogPostListPage(props: BlogPostListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<BlogPostInterface>>(
    () => `/blog-posts?params=${JSON.stringify(params)}`,
    fetcher,
  );
      ...
      : null,
    {
      ...
      cell: ({ row: { original: record } }: any) => (
        <Flex justifyContent="flex-end">
          <NextLink href={`/blog-posts/view/${record.id}`} passHref legacyBehavior>
            <Button
